module.exports = `
<head>
  <title>Admin Panel | Aero Bot List</title>
  ...
  <section class="hero">
    ...
    <div class="hero-content">
       ...
       <h1 class="hero-title">
            ...
            Aero Bot List Admin Panel
       </h1>
         
       <h2 class="hero-subtitle">
            Admin Controls of the bot list
       </h2>
...
<div class="featured text-center" id="topvoted" >
  <br />
  <br />
  <h1>Admin Options</h1>
  <br />
...
<div class="col-sm-4">
				...
				<div class="card-body">
					<h3>Approve/Deny Bots</h3>
          <input type="number" class="form-control" id="botid" placeholder="Bot ID"></textarea>
<br>
<br>
          <textarea class="form-control" id="reason" placeholder="Bot Passed Minimum Requirements." rows="5"></textarea>
<br>
<br>
          <a class="btn btn-primary" href="#" onclick="approve()">Approve</a>
          <a class="btn btn-danger" href="#" onclick="deny()">Deny</a>
				</div>
			</div>
		</div>
...
<div class="col-sm-4">
				<div class="card text-center">
				<div class="card-body">
					<h3>Certify/Uncertify Bots</h3>
          <input type="number" class="form-control" id="certbotid" placeholder="Bot ID"></textarea>
<br>
<br>
          <textarea class="form-control" readonly rows="5"></textarea>
<br>
<br>
          <a class="btn btn-primary" href="#" onclick="certify()">Certify</a>
          <a class="btn btn-danger" href="#" onclick="uncertify()">Uncertify</a>
				</div>
			</div>
		</div>
...
<div class="col-sm-4">
				...
				<div class="card-body">
					<h3>Delete Bot</h3>
          <input type="number" class="form-control" id="delbotid" placeholder="Bot ID"></textarea>
<br>
<br>
          <textarea class="form-control" id="delreason" rows="5" placeholder="Delete Bot Reason"></textarea>
<br>
<br>
          <a class="btn btn-danger" href="#" onclick="deletebot()">Delete</a>
				</div>
			</div>
		</div>
...
<div class="featured text-center" id="featured" >
  <br />
  <br />
  <h1>Approval Queue Below</h1>
  <br />
<div class="row justify-content-center mx-auto" id="featuredbots" style="width: 75%;">
{{bots}}
</div>
</div>
...
function approve(){
	if(document.getElementById("botid").value.replace(/ /g, "") == "")return alert("Bot ID must be provided!")
	if(document.getElementById("reason").value.replace(/ /g, "") == "")return alert("Approval Reason must be provided!")
...
function deny(){
	if(document.getElementById("botid").value.replace(/ /g, "") == "")return alert("Bot ID must be provided!")
	if(document.getElementById("reason").value.replace(/ /g, "") == "")return alert("Denial Reason must be provided!")
...
function certify(){
	if(document.getElementById("certbotid").value.replace(/ /g, "") == "")return alert("Certify Bot ID must be provided!")
...
$.ajax({
  ...
  url: "/certifybot",
  data: JSON.stringify({
	'botid': document.getElementById("certbotid").value
  }),
  headers: {
	"Content-Type": "application/json",
	"access_token": getCookie("access_token")
...
function deletebot(){
	if(document.getElementById("delbotid").value.replace(/ /g, "") == "")return alert("Delete Bot ID must be provided!")

	if(document.getElementById("delreason").value.replace(/ /g, "") == "")return alert("Delete Bot Reason must be provided!")
var r = confirm("Are you sure you want to delete this bot?");
if (r == false)return alert("Deletion Request Cancelled!")
  if(getCookie("access_token") == undefined)return window.location.href = "/login"
  $.ajax({
    url: '/panel/bots/' + document.getElementById("delbotid").value + '/delete',
    type: 'post',
    headers: {
        "access_token": getCookie("access_token"),
	"reason": document.getElementById("delreason").value
    },
    dataType: 'json',
    success: function (data){
        alert(data.message);
